...
func (snap *Builder) processRecord(idx int, backupCtx *BackupContext, record *importer.ScanResult, stats *scanStats, chunker *chunkers.Chunker) {
	...
	case record.Error != nil:
		...
		backupCtx.emitter.Emit("snapshot.backup.path.error", events.Error, map[string]any{
	...
	case record.Record != nil:
		...
		backupCtx.emitter.Emit("snapshot.backup.path", events.Info, map[string]any{
			"snapshot_id": snap.Header.Identifier[:],
			"path":        record.Pathname,
		})
		...
		if err != nil {
			backupCtx.emitter.Emit("snapshot.backup.file.error", events.Error, map[string]any{
				"snapshot_id": snap.Header.Identifier[:],
				"path":        record.Pathname,
				"error":       err.Error(),
			})
		...
		backupCtx.emitter.Emit("snapshot.backup.file", events.Error, map[string]any{
			"snapshot_id": snap.Header.Identifier[:],
			"path":        record.Pathname,
		})
		...
		if err := snap.processFileRecord(idx, backupCtx, record, chunker); err != nil {
			...
			backupCtx.emitter.Emit("snapshot.backup.file.ok", events.Error, map[string]any{
				"snapshot_id": snap.Header.Identifier[:],
				"path":        record.Pathname,
			})
		}
...
func (snap *Builder) importerJob(backupCtx *BackupContext) error {
	...
	backupCtx.emitter.Emit("snapshot.backup.importer.start", events.Error, map[string]any{
		"snapshot_id": snap.Header.Identifier[:],
	})
	...
	backupCtx.emitter.Emit("snapshot.backup.importer.done", events.Error, map[string]any{
		"snapshot_id": snap.Header.Identifier[:],
		"nfiles":      stats.nfiles,
		"ndirs":       stats.ndirs,
		"size":        stats.size,
...
func (snap *Builder) checkVFSCache(backupCtx *BackupContext, record *importer.ScanRecord) (*objects.CachedPath, error) {
	...
	data, err := backupCtx.vfsCache.GetCachedPath(record.Pathname)
	if err != nil || data == nil {
		return nil, err
	}

	pathinfo, err := objects.NewCachedPathFromBytes(data)
	if err != nil {
		return nil, err
	}

	same := pathinfo.Stat().Equal(&record.FileInfo)
	if record.FileInfo.Size() == -1 {
		same = pathinfo.Stat().EqualIgnoreSize(&record.FileInfo)
	}
	if !same {
		return nil, nil
	}
...
func (snap *Builder) persistVFS(backupCtx *BackupContext) (*header.VFS, *vfs.Summary, error) {
	...
	for dirPath, bytes := range diriter {
		...
		backupCtx.emitter.Emit("snapshot.backup.directory.ok", events.Error, map[string]any{
			"snapshot_id": snap.Header.Identifier[:],
			"path":        dirPath,
		})
		if dirPath == "/" {
