...
func (e *Entry) OpenSequential(fs *Filesystem) (fs.File, error) {
	return e.open(fs, true)
}

func (e *Entry) open(fs *Filesystem, sequential bool) (fs.File, error) {
	if e.FileInfo.IsDir() {
		return &vdir{
			entry: e,
			fs:    fs,
...
func (e *Entry) Xattr(fsc *Filesystem, xattrName string) (io.ReadSeeker, error) {
	...
	return NewObjectReader(fsc.repo, xattr.ResolvedObject, xattr.Size, -1), nil
}
