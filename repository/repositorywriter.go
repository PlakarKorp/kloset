...
func (r *Repository) newRepositoryWriter(cache *caching.ScanCache, id objects.MAC, typ RepositoryType, packfileTmpDir string) *RepositoryWriter {
	...
	default:
		if packfileTmpDir == "" {
			rw.PackerManager = packer.NewSeqPackerManager(rw.AppContext(), uint(r.AppContext().MaxConcurrency), &rw.configuration, rw.encode, packfile.NewPackfileInMemory, rw.GetMACHasher, rw.PutPackfile)
		} else {
			...
			rw.PackerManager = packer.NewSeqPackerManager(rw.AppContext(), uint(r.AppContext().MaxConcurrency), &rw.configuration, rw.encode, ondiskPackfileCtor, rw.GetMACHasher, rw.PutPackfile)
		}
	}
